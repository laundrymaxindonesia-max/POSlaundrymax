...
import { Store, Factory, Truck } from "lucide-react";
    ...
    >
      {ITEMS.map(({ to, label, Icon, testid }) => (
        <NavLink
          ...
          data-testid={testid}
          className={({ isActive }) =>
            `flex items-center gap-1.5 px-3 h-9 rounded-full text-[11px] font-heading font-bold uppercase tracking-wider transition-all active:scale-95 ${
              isActive
